...
/**
 * Custom dialog box to enter dates. The <code>DateChooser</code> class presents
 * a calendar and allows the user to visually select a day, month and year so
 * that it is impossible to enter an invalid date.
 **/
class DateChooser extends JDialog implements ItemListener, MouseListener,
		FocusListener, KeyListener, ActionListener {
	...
	private static final Color DAYS_FOREGROUND = Color.gray;
	/** Background color of the selected day in the calendar. */
	private static final Color SELECTED_DAY_FOREGROUND = Color.black;
	/** Text color of the selected day in the calendar. */
	private static final Color SELECTED_DAY_BACKGROUND = Color.gray;
	 ...
	 * actual user interaction is driven by the <code>DateChooser</code> class.
	 */
	...
	private static class FocusablePanel extends JPanel {
		...
		/**
		 * Always returns <code>true</code>, since <code>FocusablePanel</code>
	...
	private void construct() {
		...
		daysGrid.setBorder(BorderFactory.createEtchedBorder());
		...
		IconSets b2 = new JButtonHelper(cancel);
		b2.setIcon(IconPackBase64.CANCEL);
	}
	...
	public DateChooser(Frame owner) {
		super(owner, "Selecione a data", true);
		construct();
	}
	...
	/**
	 * Selects new date. Just calls {@link #select(Date)} with the system date
	 * as the parameter.
	 * 
